...
void button_connect_cb(void)
{
    ...
    fl_g->button_connect->color(fl_rgb_color(255, 224, 0));
...
void button_record_cb(bool ask)
{
    ...
    if ( (ask == true) && (fd = fl_fopen(cfg.rec.path, "rb")) != NULL) {
        fclose(fd);
...
void choice_cfg_bitrate_cb(void)
{
    ...
    int old_br;
    int sel_br;
    int br_list[] = {8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320};
    char text_buf[256];
...
void choice_cfg_dev_cb(void)
{
    ...
    if (cfg.audio.dev2_num >= 0 && cfg.audio.pcm_list[cfg.audio.dev2_num]->is_asio && cfg.audio.pcm_list[new_dev_num]->is_asio) {
        fl_alert(_("Primary and secondary audio device are both ASIO devices.\nYou can not use two ASIO devices at the same time.\nPlease select a different device.\n"));
...
void slider_mixer_primary_device_cb(double v)
{
    char str[10];
    snprintf(str, 10, "%+.1f dB", v);
    ...
    if (v > -0.5 && v < 0.5) {
        ...
        cfg.mixer.primary_device_gain = util_db_to_factor(v);
    }
}

void slider_mixer_secondary_device_cb(double v)
{
    char str[10];
    snprintf(str, 10, "%+.1f dB", v);
    ...
    if (v > -0.5 && v < 0.5) {
        ...
        cfg.mixer.secondary_device_gain = util_db_to_factor(v);
    }
}

void slider_mixer_streaming_gain_cb(double v)
{
    char str[10];
    snprintf(str, 10, "%+.1f dB", v);
    ...
    if (v > -0.5 && v < 0.5) {
        ...
        cfg.mixer.streaming_gain = util_db_to_factor(v);
    }
}

void slider_mixer_recording_gain_cb(double v)
{
    char str[10];
    snprintf(str, 10, "%+.1f dB", v);
    ...
    if (v > -0.5 && v < 0.5) {
        ...
        cfg.mixer.recording_gain = util_db_to_factor(v);
    }
}
void slider_mixer_master_gain_cb(double v)
{
    char str[10];
    snprintf(str, 10, "%+.1f dB", v);
    fl_g->text_mixer_master_volume->copy_label(str);
    ...
    if (v > -0.5 && v < 0.5) {
        ...
        cfg.main.gain = util_db_to_factor(v);
        fl_g->slider_gain->value(v);
    }
    // Without redrawing the main window the slider knob is not redrawn correctly
    fl_g->window_main->redraw();
}
...
void slider_mixer_cross_fader_cb(double v)
{
    ...
    if (cfg.mixer.cross_fader == 0) {
        ...
        cfg.mixer.primary_X_fader = 1.0;
        cfg.mixer.secondary_X_fader = 1 + (v / 100.0);
    }
    else {
        cfg.mixer.primary_X_fader = 1 - (v / 100.0);
        cfg.mixer.secondary_X_fader = 1.0;
    }
...
{
    slider_mixer_primary_device_cb(0);
    fl_g->slider_mixer_primary_device->value(0);

    slider_mixer_secondary_device_cb(0);
    fl_g->slider_mixer_secondary_device->value(0);

    slider_mixer_streaming_gain_cb(0);
    fl_g->slider_mixer_streaming_gain->value(0);

    slider_mixer_recording_gain_cb(0);
    fl_g->slider_mixer_recording_gain->value(0);
    
    slider_mixer_master_gain_cb(0);
    fl_g->slider_mixer_master_gain->value(0);

    slider_mixer_cross_fader_cb(0);
    fl_g->slider_mixer_cross_fader->value(0);
