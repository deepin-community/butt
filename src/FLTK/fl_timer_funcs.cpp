...
pthread_t request_listener_count_thread;
static int g_listener_count = -1;

int g_vu_meter_timer_is_active = 0;
int g_stop_vu_meter_timer = 0;

void set_threshold_input_from_command(command_t *command, Fl_Value_Input *input)
{
    if (command->param != NULL) {
        input->value(*(int *)command->param);
        input->do_callback();
        free(command->param);
    }
}
...
void cmd_timer(void *)
{
    ...
    case CMD_SET_STREAM_SILENCE_THRESHOLD:
        set_threshold_input_from_command(&command, fl_g->input_cfg_silence);
        Fl::repeat_timeout(0.25, &cmd_timer);
        break;
    case CMD_SET_RECORD_SIGNAL_THRESHOLD:
        set_threshold_input_from_command(&command, fl_g->input_rec_signal);
        Fl::repeat_timeout(0.25, &cmd_timer);
        break;
    case CMD_SET_RECORD_SILENCE_THRESHOLD:
        set_threshold_input_from_command(&command, fl_g->input_rec_silence);
        Fl::repeat_timeout(0.25, &cmd_timer);
        break;
    case CMD_GET_STATUS:
...
int request_listener_count_thread_running = 0;
void *request_listener_count_thread_func(void *listener_count)
{
    request_listener_count_thread_running = 1;
    ...
    request_listener_count_thread_running = 0;
...
void request_listener_count_timer(void *reset)
{
    ...
    if (request_listener_count_thread_running == 0) {
        listener_count = -2;
        pthread_create(&request_listener_count_thread, NULL, request_listener_count_thread_func, &listener_count);
    }
...
void split_recording_file(void)
{
    ...
    if ((next_fd = fl_fopen(cfg.rec.path, "rb")) != NULL) {
        fclose(next_fd);
